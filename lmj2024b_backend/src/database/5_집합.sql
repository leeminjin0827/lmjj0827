...
select * from table1 , table2;		# 2개 이상의 테이블을 조회 : select * from 테이블명A , 테이블명B;
...
# [*2*] inner joun ~ on절 이용한 교집합 표현하기 , 가독성 , where 일반조건 , on 집합조건 , having : 그룹조건
# select * from 테이블명A inner join 테이블B on 집합조건( 테이블A.PK속성명 = 테이블B.FK속성명 )
...
# [4] using 키워드 : 전제조건은 pk속성명과 fk속성명이 같아야 한다. 현재 예제는 num_pk/num_pk(FK) 다르므로
...
# OUTER JOIN
# [6] 왼쪽테이블(tabel1) 에 있는 모든 레코드를 포함하고 오른쪽 테이블(table2)에 교집합 되는 레코드 조회 , 없으면 null
...
# 속성명 in null : 속성값이 null인지 비교연산 , 속성명 is not null : 속성값이 null이 아닌지 비교연산

# 실습1 : 아래 테이블 코드를 참조하여 문제를 풀이하시오.
# [문제1] : 모든 제품(product)테이블 레코드 조회시 모든 속성명을 표시하며 속성명이 '카테고리번호' 대신에 '카테고리명'을 조회하는 SQL 작성하시오.
...
# 별칭 : 속성명 별칭명 , 테이블명 별칭명( as 생략가능 )
select p.제품번호_pk , p.제품명 , p.제품가격 , p.카티고리명
	from product as p inner join pcategory as pc on p.카테고리번호_fk = pc.카테고리번호_pk;
    
# [문제2] : 제품번호 1번의 재고수량 합계를 조회하는 SQL 작성하시오. 그리고 조회시 '제품명' 과 '합계수량' 속성명 만 조회 하시오.
select * from stock;								# 재고 테이블 조회
select * from stock where 제품번호_fk = 1;				# 제품번호 1번 인 레코드만 조회
select sum( 재고수량 ) from stock where 제품번호_fk = 1;	# 제품번호 1번 인 레코드의 재고수량 속성 총합계
