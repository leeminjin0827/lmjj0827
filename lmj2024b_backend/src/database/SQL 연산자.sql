...
# [5] 속성값에 산술연산
# 산술연산자 : 속성명 + 값 , 속성명 - 값 , 속성명 * 3 , 속성명 / 값 , 속성명 / div 값 , 속성명 mod 값
select mnumber 인원수 , mnumber+3 as 더하기 , mnumber-3 as 빼기 , mnumber*3 as 곱셈 , mnumber/3 as 나누기 ,
...
# 문자열패턴 : 속성명 like '문자패턴'
#		% : 모든 문자 대응				ex] 삼성전자/LG전자/뮬전자 , %전자 -> 삼성전자/LG전자/뮬전자
#		_ : _개수 만큼 문자 대응			ex] 삼성전자/LG전자/뮬전자 , _전자 -> 뮬전자
...
select * from member where mheight >= 165 and mheight <= 170;	# 'mheight'속성값이 165 이상 이면서 170 이하 인 레코드 조회
select * from member where mheight between 165 and 170;			# 'mheight'속성값의 165 ~ 170 사이 인 레코드 조회
# 'maddr'속성값이 '경기' 이거나 '전남' 이거나 '경남' 인 레코드 조회
...
select * from member where mname like '%핑크';					# 'mname'속성값이 '에이' 로 끝나는 레코드 조회
...
# [7] 정렬 , 조회 결과의 레코드를 득점한 필드 기준으로 정렬
# order by 속성명 asc		, [기본값/생략가능] 	, 지정한 속성명 기준으로 속성값들을 오름차순 정렬, 작은값 -> 큰값 , 과거날짜 -> 최근날짜 , ABC
# order by 속성명 desc					, 지정한 속성명 기준으로 속성값들을 내림차순 정렬, 큰값 -> 작은값, 최근날짜 -> 과거날짜
# order by 속성명1 정렬기준 , 속성명2 정렬기준 	, 지정한 속성명의 정렬이 2개 이상일때 , (쉼표)구분하기 , 2차정렬은 1차정렬내 동일한 값
...
# 실습1 : 주소가 '서울'인 레코드를 데뷔일 기준으로 내림차순으로 조회 결과 상위 2개만 조회 하시오.
select * from member;													# 'member' 테이블 전체 레코드 조회
select * from member where maddr = '서울';								# 'member' 테이블의 'maddr' 속성명이 '서울' 인 레코드 조회
select * from member where maddr = '서울' order by mdebut desc;			# + 'mdebut' 속성명 기준오름차순 조회
...
# 1. 회원아이디 별 구매수량 총합계
select mid , sum( bamount ) from buy group by mid;	# 'mid' 속성명의 값으로 (중복)그룹 하고 'bamount' 속성명으로 집계한 레코드 조회
...
# 1. 구매수량( bamount )이 3 초과 하고 회원아이디별 레코드 조회
...
select mid , bprice 구매금액 from buy where 총구매금액 >= 1000; # 오류 : where 별칭으로 조건속성명 불가능
select mid , sum( bamount * bprice ) as 총구개금액 where 총구매금액 >= 1000; # 오류 : 집계 후 조건은 where 불가능

# 실습2 : 구매수량(bamount) 이 2개 초과 인 레코드 에서 회원아이디별 총구매금액 가격이(bprice)평균 이 50 이상인 레코드 조회
# + 구매가격평균으로 내림차순 조회 , 상위 2개만 조회
# 1. buy 테이블에서 구매수량(bamount) 이 2개 초과 인 레코드
select * from buy where bamount > 2;
# 2. 총구매금액 가격이(bprice)평균 * ~~별 집계(합계/평균/개수)
select
